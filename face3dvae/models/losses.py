...
class VAELoss(nn.Module):
    ...
    def forward(
        ...
        if self.perceptual_loss_weight is not None and self.perceptual_loss_weight > 0.0 and not no_perceptual:
            ...
            full_recon_loss = recon_loss + self.perceptual_loss_weight * perceptual_loss

        nll_loss = full_recon_loss / torch.exp(self.logvar) + self.logvar
        ...
        return nll_loss, weighted_nll_loss, weighted_kl_loss, torch.mean(recon_loss), torch.mean(perceptual_loss)
