...
class ReferenceAttentionControl:
    ...
    def register_reference_hooks(
        ...
        def hacked_temporal_transformer_inner_forward(
            ...
            if len(self.bank) > 0:
                ...
                #addition = addition[total_bs//5:]
                if torch.sum(torch.abs(addition)) < 1e-5:
                    ...
                    addition = addition.permute(0, 2, 1, 3, 4)
        ...
        if self.reference_attn:
            ...
            if self.mode == 'read':
                motion_modules = [
                    module for module in torch_dfs(self.unet.down_blocks[3]) + torch_dfs(self.unet.up_blocks[3])
                                  if isinstance(module, TransformerTemporalModel)
                ]
                for i, module in enumerate(motion_modules):
                    module._original_inner_forward = module.forward
    ...
    def update(self, writer, dtype=torch.float16):
        ...
        writer_res_motion_modules = [
            module for module in torch_dfs(writer.unet.down_blocks[3]) + torch_dfs(writer.unet.up_blocks[3])
                          if isinstance(module, ResnetBlock2D)
        ]
...
class ReferenceNet3D(nn.Module):
    '''
    ...
    def forward(
        ...
        self.referencenet(
            ...
            encoder_hidden_states=ref_encoder_hidden_states.repeat_interleave(ref_latents.size(0), dim=0),
            return_dict=False,
        )
